...
export function WeightHistory({ 
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      change: sortedLogs.length > 1 
        ? Math.round((sortedLogs[sortedLogs.length - 1].weight - sortedLogs[0].weight) * 10) / 10 
        : 0
    };
  }, [filteredLogs]);
  ...
  const chartData = useMemo(() => {
    ...
    const availableLogs = Array.from(dailyMap.values()).sort((a, b) => a.dateKey.localeCompare(b.dateKey));
  ...
  if (weightLogs.length === 0) {
    return (
      <div className="text-center py-8 space-y-3">
        ...
        <p className="text-sm text-gray-400">体重を記録してグラフを見てみましょう</p>
      </div>
    );
  }
      ...
      <div className="space-y-2">
        <h3 className="text-lg font-semibold text-gray-700 flex items-center gap-2">
          ...
          <span className="text-sm font-normal text-gray-500">
            ({selectedPeriod === 'all' ? '全期間' : periodOptions.find(opt => opt.value === selectedPeriod)?.label})
          </span>
        </h3>
            ...
            >
              ...
              <div className="text-right">
                ...
                <div className="text-xs text-gray-400">
                  {log.measurementTiming === 'morning' ? '朝' : 
                   log.measurementTiming === 'night' ? '夜' : 'その他'}
                </div>
              </div>
            </div>
          ))}
